...
float4 PixelShaderFunction(VertexShaderOutput input) : COLOR0
{
    ...
    float4 fadeMapColor = tex2D(uImage1, float2(frac(coords.x * 2 - time * 0.5), coords.y));
    float opacity = (fadeMapColor.r * 2) * bloomFadeout;
    
    float4 finalColor = lerp(color, float4(uColor, 1), fadeMapColor.r - 0.8);
    
    // Fade out at the ends of the streak.
    if (coords.x < 0.05)
        opacity *= pow(coords.x / 0.05, 3);
    if (coords.x > 0.95)
