...
{
    public class ScytheSummoningPortal : ModProjectile
    {
        ...
        {
            projectile.rotation += (projectile.identity % 2f == 0f).ToDirectionInt() * 0.1f;
            projectile.Opacity = Utils.InverseLerp(0f, 40f, projectile.timeLeft, true);

            if (Main.netMode != NetmodeID.MultiplayerClient && Time % 105f == 0f)
            {
