...
namespace InfernumMode.BehaviorOverrides.BossAIs.Yharon
{
    public class DragonFireball : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            // Only do this 4 times.
            if (Timer < 4)
            { 
                for (int j = 0; j < 5; j++)
                {
                    Color fireColor = Color.Lerp(Color.Orange, Color.White, Main.rand.NextFloat(0.1f, 0.9f));
                    float rotationAmount = Main.rand.NextFloat(0.02f, 0.04f);
                    FireballParticle fire = new(Projectile.Center, Projectile.velocity, fireColor, 180, Main.rand.NextFloat(0.057f, 0.063f), 1f, (Main.rand.NextBool() ? -1 : 1) * rotationAmount);
