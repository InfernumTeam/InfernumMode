...
namespace InfernumMode.Content.BehaviorOverrides.BossAIs.OldDuke
{
    public class OldDukeBehaviorOverride : NPCBehaviorOverride
    {
            ...
            if (attackTimer == 31f)
            {
                ...
                if (Main.netMode != NetmodeID.MultiplayerClient)
                {
                    ...
                    for (int i = 0; i < 4; i++)
                    {
                        ...
                        int shark = NPC.NewNPC(npc.GetSource_FromAI(), (int)spawnPosition.X, (int)spawnPosition.Y, ModContent.NPCType<SulphurousSharkron>(), 0, 0f, 1f);
        ...
        public override IEnumerable<Func<NPC, string>> GetTips(bool hatGirl)
        {
            ...
            {
                if (hatGirl)
                    return "Be sure to remember if any sharkrons or tooth balls appear, so that their remains don't surprise you later!";
                return string.Empty;
            };
            yield return n =>
            {
                if (hatGirl)
                    return "Movement speed is gonna matter a lot against that fish!";
                return string.Empty;
            };

            yield return n =>
            {
                if (n.life < n.lifeMax * Phase4LifeRatio && hatGirl)
                    return "Try to keep a counter in the last phase. The Old Duke will teleport, charge seven times, and then repeat.";
                return string.Empty;
            };
            yield return n =>
            {
                if (!Main.LocalPlayer.HasDash() && hatGirl)
