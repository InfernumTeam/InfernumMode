...
namespace InfernumMode.Content.Tiles.Abyss
{
    public class AbyssalKelp : ModTile
    {
        ...
        public override void KillTile(int i, int j, ref bool fail, ref bool effectOnly, ref bool noItem)
        {
            ...
            if (tile1.HasTile && tile1.TileType == Type)
            {
                WorldGen.KillTile(i, j - 1, false, false, false);
                if (!tile1.HasTile && Main.netMode != NetmodeID.SinglePlayer)
                    NetMessage.SendData(MessageID.TileManipulation, -1, -1, null, 0, i, j - 1);
            }

            if (tile2.HasTile && tile2.TileType == Type)
            {
                tile2.Get<TileWallWireStateData>().TileFrameX = (short)(WorldGen.genRand.Next(6) * 18);
                tile2.Get<TileWallWireStateData>().TileFrameY = 0;
