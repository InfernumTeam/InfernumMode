...
float4 PixelShaderFunction(VertexShaderOutput input) : COLOR0
{
    ...
    float4 fadeMapColor = tex2D(uImage1, float2(frac(coords.x + sin(uTime + 1.57) * 0.01), frac(coords.y + uTime * 1.4)));
    ...
    color.rgb = lerp(color.rgb, transformColor, opacity);
    return color * opacity * 1.4;
}
