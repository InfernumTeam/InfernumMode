...
{
    public class AcidicTidalWave : ModProjectile
    {
        ...
        {
            DisplayName.SetDefault("Acidic Wave");
        ...
        public override void AI()
        {
            ...
            if (WaveHeight < 60f)
                WaveHeight = 60f;
            WaveHeight = MathHelper.Lerp(WaveHeight, 1800f, 0.04f);
            projectile.Opacity = (float)Math.Sin(projectile.timeLeft / 360f * MathHelper.Pi) * 2f;
		...
		{
            return Color.Lerp(Color.LimeGreen, Color.MediumPurple, (float)Math.Abs(Math.Sin(completionRatio * MathHelper.Pi + Main.GlobalTime)) * 0.2f) * projectile.Opacity;
		}
