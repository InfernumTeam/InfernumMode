...
using InfernumMode.FuckYouModeAIs.Guardians;
...
namespace InfernumMode.FuckYouModeAIs.Twins
{
	public static class TwinsAttackSynchronizer
    {
        ...
        public static bool DoAI(NPC npc)
        {
            ...
            if (alone)
            {
                if (hasStartedHealFlag == 1f && overdriveTimer < TwinsShield.HealTime)
                {
                    npc.velocity *= 0.97f;
                    ...
                    if (healCountdown <= 0f)
                    {
                        if (overdriveTimer == TwinsShield.HealTime - 100)
                        ...
                        if (Main.netMode != NetmodeID.MultiplayerClient && overdriveTimer == TwinsShield.HealTime - 15)
                        {
            ...
            {
                case TwinsAttackState.ChargeRedirect:
                    Vector2 destination = Target.Center - Vector2.UnitY * 280f;
                    destination.X += isSpazmatism.ToDirectionInt() * 520f;
                    ...
                    else
                    {
                        ...
                        if (Main.netMode != NetmodeID.MultiplayerClient && UniversalAttackTimer % 30 == 29)
                        {
                            if (isSpazmatism)
                            {
                                ...
                                for (int i = 0; i < 2; i++)
                                {
                                    Vector2 shootVelocity = npc.DirectionTo(Target.Center).RotatedBy(MathHelper.Lerp(-0.25f, 0.25f, i / 2f)) * shootSpeed;
                                    Utilities.NewProjectileBetter(npc.Center + shootVelocity * 4f, shootVelocity, ProjectileID.CursedFlameHostile, 85, 0f);
                                }
                            }
                            else
                            {
                                ...
                                Utilities.NewProjectileBetter(npc.Center + shootVelocity * 4f, shootVelocity, ProjectileID.DeathLaser, 75, 0f);
                            }
                        }
                    }
                    break;
                ...
                case TwinsAttackState.MatisLordEsqueCharge:
                    ...
                    if (UniversalAttackTimer % (redirectTime + chargeTime) <= redirectTime)
                    {
                        ...
                        if (UniversalAttackTimer % (redirectTime + chargeTime) == redirectTime && willCharge)
                        {
                            ...
                            float chargeSpeed = MathHelper.Lerp(14f, 21f, 1f - CombinedLifeRatio);
                ...
                case TwinsAttackState.Spin:
                    ...
                    if (UniversalAttackTimer >= 60 && UniversalAttackTimer <= 300)
                    {
                        ...
                        if (Main.netMode != NetmodeID.MultiplayerClient && UniversalAttackTimer % fireRate == fireRate - 1)
                        {
                            ...
                            int proj = Utilities.NewProjectileBetter(npc.Center + shootVelocity * 4f, shootVelocity, projectileType, 85, 0f);
        ...
        internal static void DoAI_RetinazerAlone(NPC npc)
        {
            ...
            switch ((RetinazerAttackState)(int)attackState) 
            {
                ...
                case RetinazerAttackState.DanceOfLightnings:
                    ...
                    if (Main.netMode != NetmodeID.MultiplayerClient && attackTimer >= 270f && attackTimer % 10f == 9f && npc.velocity.Length() > 13f)
        ...
        internal static void DoAI_SpazmatismAlone(NPC npc)
        {
            ...
            switch ((SpazmatismAttackState)(int)attackState)
			{
                case SpazmatismAttackState.MobileChargePhase:
                    ...
                    int chargeTime = (int)MathHelper.SmoothStep(65f, 45f, 1f - lifeRatio);
                    int slowdownTime = 45;
                    ...
                    // Fade out quickly when charging.
                    if (attackTimer >= hoverTime + chargeDelayTime)
                    {
                        npc.damage = 0;
                        float idealOpacity = MathHelper.Lerp(0.3f, 0.1f, Utils.InverseLerp(0.45f, 0.1f, lifeRatio, true));
					...
					{
                        npc.Opacity = MathHelper.Lerp(npc.Opacity, 1f, 0.2f);
                        npc.velocity *= 0.94f;
                ...
                case SpazmatismAttackState.ShadowflameCarpetBomb:
                    ...
                    float carpetBombSpeed = MathHelper.SmoothStep(14f, 21.5f, 1f - lifeRatio);
                    if (attackTimer < redirectTime)
					{
                        ...
                        if (npc.WithinRange(destination, 24f))
                        {
                    ...
                    if (attackTimer == redirectTime)
					{
                        float carpetBombChargeSpeed = MathHelper.SmoothStep(13f, 18f, 1f - lifeRatio);
